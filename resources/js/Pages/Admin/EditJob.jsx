...
export default function EditJob({ job }) {
    // Parse salary range to extract currency, amount, and period
    const parseSalaryRange = (salaryRange) => {
        ...
        // Example: "NGN 20000000 / year" or "$50,000 / month"
        const match = salaryRange.match(/^(\w+)\s*([\d,]+)\s*\/\s*(\w+)$/);
        if (match) {
            return {
                salary_currency: match[1],
                salary_amount: match[2].replace(/,/g, ''),
                salary_period: match[3]
            };
        }
    ...
    const parsedSalary = parseSalaryRange(job.salary_range);
    ...
    const [formData, setFormData] = useState({
        ...
        salary_amount: parsedSalary.salary_amount,
        salary_period: parsedSalary.salary_period,
    ...
    const handleSubmit = (e) => {
        ...
        router.patch(`/Admin/jobs/${job.id}`, submitData, {
            onSuccess: () => {
                setProcessing(false);
