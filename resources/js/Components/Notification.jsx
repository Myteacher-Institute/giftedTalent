...
export default function Notification({ className = '', newJobsCount = 0, onMarkAsSeen }) {
    ...
    // Combine existing notifications count with new jobs count
    const totalUnreadCount = notifications.unread_count + newJobsCount;

    const bellIcon = totalUnreadCount > 0
    ...
    const handleOpenDropdown = () => {
        setIsOpen(true);
        // Clear new jobs badge when user opens notifications
        if (newJobsCount > 0 && onMarkAsSeen) {
            onMarkAsSeen();
        }
    };
        ...
        <div className={`relative ${className}`}>
            <Dropdown>
                ...
                <Dropdown.Content className="w-80 max-h-96 overflow-y-auto">
                        ...
                        <div className="p-4 border-b border-yellow-200 bg-yellow-50">
                            <div className="flex items-start space-x-3">
                                <div className="flex-shrink-0">
                                    <span className="inline-flex h-8 w-8 items-center justify-center rounded-full text-sm font-medium bg-yellow-100 text-yellow-800">
                                        🎯
                                    </span>
                                </div>
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-sm text-gray-600 mt-1">
                                        Check out the latest opportunities that match your profile.
                                    </p>
                    ...
                    {notifications.recent_unread.length === 0 && newJobsCount === 0 ? (
