...
class ProfileController extends Controller
{
    ...
    public function updateExtendedProfile(Request $request): RedirectResponse
    {
        try {
            ...
            // Update the existing profile or create if doesn't exist
            if (!$profile) {
                $profile = Profile::create(array_merge(['user_id' => $user->id], $filteredProfileData));
            } elseif (!empty($filteredProfileData)) {
     ...
     */
    public function getJobPreferences(Request $request): \Illuminate\Http\JsonResponse
    {
        try {
            $user = Auth::user();
    ...
    public function addSkill(Request $request): RedirectResponse
    {
        try {
            ...
            // Attach skill to user if not already attached
            ...
            return Redirect::back()->withErrors(['error' => 'Failed to add skill. Please try again.']);
        }
    }
    ...
    public function removeSkill(Request $request, int $skillId): RedirectResponse
    {
        try {
            ...
            return Redirect::back()->withErrors(['error' => 'Failed to remove skill. Please try again.']);
        }
    }
    ...
    public function addExperience(Request $request): RedirectResponse
    {
        try {
            ...
            return Redirect::back()->withErrors(['error' => 'Failed to add work experience. Please try again.']);
        }
    }
     ...
     * Delete a resume.
     */
    public function destroyResume($id): RedirectResponse
    {
        try {
